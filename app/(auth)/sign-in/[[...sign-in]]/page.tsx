import { Metadata } from "next";
import Image from "next/image";
import { SignIn } from "@clerk/nextjs";
import { logoIcon } from "@/public/_static";
import { Check } from "lucide-react";

export const metadata: Metadata = {
  title: "Sign In — Creatify AI",
  description: "Sign in to your Creatify AI account",
};

const platforms = [
  "Facebook post descriptions",
  "YouTube video ideas",
  "Instagram hashtags",
  "TikTok hashtags",
  "LinkedIn posts",
  "Twitter / X posts",
];

export default function SignInPage() {
  return (
    <div className="grid min-h-screen lg:grid-cols-2">
      {/* Left brand panel — hidden on mobile */}
      <div className="brand-gradient hidden flex-col justify-between p-12 text-white lg:flex">
        <div className="flex items-center gap-2.5">
          <Image
            src={logoIcon}
            height={36}
            width={36}
            alt="Creatify AI"
            className="rounded-lg"
          />
          <span className="font-urban text-xl font-extrabold">Creatify_AI</span>
        </div>

        <div className="space-y-2">
          <blockquote className="mb-8">
            <p className="text-lg font-medium leading-relaxed">
              &ldquo;Generate scroll-stopping social content in seconds with AI.
              Platform-ready output for every major network.&rdquo;
            </p>
          </blockquote>
          {platforms.map((p) => (
            <div key={p} className="flex items-center gap-2.5 text-sm">
              <div className="flex size-5 shrink-0 items-center justify-center rounded-full bg-white/20">
                <Check className="size-3" />
              </div>
              {p}
            </div>
          ))}
        </div>

        <p className="text-sm text-white/60">
          © {new Date().getFullYear()} Creatify AI. All rights reserved.
        </p>
      </div>

      {/* Right: Clerk form */}
      <div className="flex items-center justify-center bg-background p-8">
        <div className="w-full max-w-sm space-y-6">
          {/* Mobile logo */}
          <div className="flex justify-center lg:hidden">
            <Image
              src={logoIcon}
              height={40}
              width={40}
              alt="Creatify AI"
              className="rounded-xl"
            />
          </div>
          <SignIn />
        </div>
      </div>
    </div>
  );
}
